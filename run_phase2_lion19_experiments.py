...
APPROACH_SCRIPTS = {
    ...
    'lion19': 'main_alldiff_lion19.py',
}


def run_phase2_for_benchmark(benchmark_name, phase1_pickle_path, output_dir, log_file, *, approach='cop', extra_args=None):
    ...
    script_path = APPROACH_SCRIPTS.get(approach)
    if script_path is None:
        raise ValueError(f"Unsupported approach '{approach}'. Supported approaches: {', '.join(APPROACH_SCRIPTS)}")
...
def create_summary_report(all_results, output_dir, summary_basename='phase2_summary.txt', json_basename='phase2_results.json'):
    
    summary_file = os.path.join(output_dir, summary_basename)
    json_file = os.path.join(output_dir, json_basename)
    ...
    with open(summary_file, 'w', encoding='utf-8') as f:
        ...
        for result in all_results:
            ...
            if result.get('approach'):
                f.write(f"  Approach: {result['approach']}\n")
...
def main():
    parser = argparse.ArgumentParser(description="Phase 2 batch runner for AllDifferent experiments")
    parser.add_argument('--approach', choices=APPROACH_SCRIPTS.keys(), default='cop', help="Phase 2 refinement approach to run")
    parser.add_argument('--output_dir', type=str, default=None, help="Directory to store logs and summaries")
    parser.add_argument('--benchmarks', nargs='*', default=None, help="Subset of benchmarks to run")
    parser.add_argument('--script-args', nargs=argparse.REMAINDER, help="Additional arguments passed to the underlying phase 2 script (prefix with --)")
    ...
    if args.benchmarks:
        name_map = {bench['name']: bench for bench in benchmarks}
        selected = []
    ...
    if not benchmarks:
        print("[ERROR] No benchmarks selected to run")
        return 1

    if args.output_dir:
        output_dir = args.output_dir
    else:
        output_dir = 'phase2_output' if args.approach == 'cop' else os.path.join('phase2_output', args.approach)
    ...
    extra_args = args.script_args if args.script_args else None
    ...
    for i, bench in enumerate(benchmarks, 1):
        ...
        log_file = os.path.join(output_dir, f"{bench['name']}_{args.approach}_phase2.log")
    ...
    summary_file, json_file = create_summary_report(
        all_results,
        output_dir,
        summary_basename=f"{args.approach}_phase2_summary.txt",
        json_basename=f"{args.approach}_phase2_results.json"
    )
    ...
    for result in all_results:
        ...
        print(f"  {status_symbol} {result['benchmark']} ({result.get('approach', args.approach)}): {result['status']}")
        if result.get('queries') is not None:
