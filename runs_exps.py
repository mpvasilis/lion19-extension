...
def main(num_runs=10):
    ...
    with open(report_path, 'w') as f:
        ...
        f.write("  MQuQ: Active learning queries (Phase 3) (mean±std)\n")
        ...
        f.write("  MQuT(s): Duration of active learning phase (mean±std)\n")
    ...
    with open(detailed_report_path, 'w') as f:
        f.write(f"Solution Variance Experiment Results (COP vs LION Comparison) - ALL INDIVIDUAL RUNS\n")
        f.write("="*140 + "\n\n")
    ...
    comparison_path = f"{output_dir}/cop_vs_lion_comparison_aggregated.txt"
    with open(comparison_path, 'w') as f:
        f.write(f"COP vs LION Comparison Summary (Aggregated over {num_runs} runs)\n")
        f.write("="*140 + "\n\n")
        ...
        for (benchmark, sols), approaches in sorted(grouped.items()):
            ...
            if cop and lion:
                ...
                # Accuracy metrics
                f.write(f"{'Precision (%)':<25} {cop.get('precision', 'N/A'):<20} {lion.get('precision', 'N/A'):<20} ")
                if 'precision_mean' in cop and 'precision_mean' in lion:
                    diff = lion['precision_mean'] - cop['precision_mean']
                    f.write(f"{diff:<15.2f}\n")
                else:
                    f.write(f"{'N/A':<15}\n")

                f.write(f"{'Recall (%)':<25} {cop.get('recall', 'N/A'):<20} {lion.get('recall', 'N/A'):<20} ")
                if 'recall_mean' in cop and 'recall_mean' in lion:
                    diff = lion['recall_mean'] - cop['recall_mean']
                    f.write(f"{diff:<15.2f}\n")
                else:
                    f.write(f"{'N/A':<15}\n")
    ...
    # Save detailed JSON with both individual and aggregated results
    ...
    print(f"\n{'='*80}")
    print(f"FINAL STATISTICS (MULTI-RUN EXPERIMENTS)")
    print(f"{'='*80}")
