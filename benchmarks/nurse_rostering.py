...
def construct_nurse_rostering(shifts_per_day=3, num_days=7, num_nurses=8, nurses_per_shift=2, max_workdays=5):
    ...
    # Constraint: Each nurse works at most 'max_workdays' days per week
    # Since a nurse appears at most once per day (due to AllDifferent), the inner sum is 0/1.
    for nurse in range(1, num_nurses + 1):
        model += cp.sum([cp.sum(roster_matrix[d, ...] == nurse) for d in range(num_days)]) <= max_workdays

    # Binary constraints between nurse assignments
                ...
                for day2 in range(day1, num_days):
                    ...
                    for shift2 in range(start_shift2, shifts_per_day):
                        ...
                        for pos2 in range(start_pos2, nurses_per_shift):
                            ...
                            # Night shift nurses should not work morning shift next day
                            if (shift1 == shifts_per_day - 1 and day2 == day1 + 1 and shift2 == 0):
                                model += [nurse1 != nurse2]
                            
                            # Same nurse should not work consecutive shifts on same day
                            if (day1 == day2 and abs(shift1 - shift2) == 1):
                                model += [nurse1 != nurse2]
                            
                            # Nurses should have at least one day break between working days
                            if (day2 == day1 + 1 and shift1 == shifts_per_day - 1 and shift2 == 0):
                                model += [nurse1 != nurse2]
                            
                            # Weekend shifts should be distributed fairly
                            if (day1 >= 5 and day2 >= 5 and day1 != day2):  # Weekend days
                                model += [nurse1 != nurse2]
                            
                            # First and last positions in shifts should be experienced nurses
                            if (pos1 == 0 and pos2 == nurses_per_shift - 1 and 
                                day1 == day2 and shift1 == shift2):
                                model += [nurse1 < nurse2]  # Assuming lower IDs are more experienced

    # # Check for feasibility before proceeding
    # if not model.solve():
    #     raise Exception("The problem has no solution")

    # Store the constraints from the model
    C_T = list(model.constraints)

    # Create the language using abstract variables (as many as maximum arity)
    AV = absvar(2)
    lang = [
        AV[0] == AV[1],
        AV[0] != AV[1],
...
# Uncomment below to test the model
# if __name__ == "__main__":
#     instance, oracle = construct_nurse_rostering()
#     print(oracle.constraints)
